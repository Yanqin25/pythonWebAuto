...
class ExcelUtil():
    ...
    def get_data(self):
     ...
     # 根据指定列的值获取前多少列数据
    def get_data_by_colname(self,name,index,num):
        rows=self.get_rows()
        return [self.sheet.row_values(i)[:num] for i in range(1,rows) if self.get_cell_val(i,index)==name] if rows > 1 else None
    def get_rows(self):
...
class ExcelUtilXlsx():
    ...
    def __init__(self, file_path=os.path.join(os.getcwd(), "sourceData/registerCase.xls"), index=0):
...
if __name__ == "__main__":
    ...
    # 写入xlsx
    ex = ExcelUtilXlsx(r"K:\TEST-DOCUMENT\04.测试用例\后台Vue改造\测试用例-图片库-待评审-杨勤.xlsx", 1)
